...
def create(data):
    ...
    superuser = os.getenv("PG_SUPERUSER", "postgres")  # Default to "postgres" if not specified
    superpass = os.getenv("PGPASSWORD")
    
    if not superpass:
        print("[red]ERROR: Please set PGPASSWORD to your Postgres superuser password.[/red]")
        sys.exit(1)
